...
## Rapid file-activation correction

Manual acceptance on 2026-09-11 against `/home/gzq/AndroidStudioProjects/superboost` showed that the earlier four-file Chromium journey did not cover two coupled activation failures. Files-tree selection changed before the editor session tried to open a document, while the 20-state ceiling refused the twenty-first file even when old tabs were clean. This left the new row selected and the previous editor visible, which was indistinguishable from a renderer stall. The retained-state implementation also destroyed and reconstructed the one mounted CodeMirror view on every ordinary source-file switch. A parser package that resolved after its XML tab became inactive could still reconfigure and parse that hidden state on the UI thread while a Java or Kotlin selection was mounting.

The state ceiling now retires the oldest inactive clean tab and admits the requested document. Dirty buffers and in-flight saves remain non-evictable; when all 20 entries are protected, the open is rejected explicitly and tree selection returns to the document that remains visible. Ordinary text-file transitions preserve the single mounted `EditorView` and exchange its retained `EditorState`. Lazy language installation yields until the plain document has received a paint opportunity, and losing activation cancels both the request identity and the pending installation. Switching to Diff, Markdown presentation, loading, or error surfaces still destroys the text view because those surfaces have different ownership and layout.

A production-like Chromium journey made 60 sequential Files-tree selections across four source files after the correction. It converged on `styles.css`, displayed the expected `:root` content, reported CSS language support ready, retained four text tabs, mounted exactly one CodeMirror editor, and emitted no warning or error. Session fixtures separately fill all 20 slots, prove automatic clean-tab retirement and final activation of the twenty-first file, and prove that a new file remains blocked when every retained buffer is dirty. The release executable also remained alive for six seconds while opening the reported `superboost` repository through an isolated application profile, followed by the ordinary disposable-repository smoke.

One absolute Node fixture loaded and fully parsed representative files from the reported repository. The 206,444-code-unit `vaLib/src/main/AndroidManifest.xml` measured 6.5 milliseconds for language loading, 10.3 milliseconds for state construction, and 46.7 milliseconds for a complete parse. The 266,566-code-unit `StringUtils.java` measured 4.2, 3.8, and 69.7 milliseconds respectively; the 59,981-code-unit `MainActivity.java` measured 0.0, 2.9, and 43.5 milliseconds after the Java package was cached; and the 3,608-code-unit `AdjustHelper.kt` measured 5.3, 7.3, and 0.9 milliseconds. These are one-run Node full-parse observations rather than installed WebKit frame times or a before/after series. They demonstrate that obsolete parser work is large enough to avoid, but general interaction-performance impact remains **inconclusive** until manual Linux/macOS acceptance or an installed frame-time trace.

The complete frontend suite passes 147 tests, TypeScript checking and the production build pass, and the Rust workspace passes formatting, strict all-target Clippy, and 65 tests. The main JavaScript increase comes from the explicit view-transfer and activation policy; no parser, worker, watcher, index, service process, repository query, or retained-state ceiling is added.

| Output | Font-selector baseline | Activation correction | Movement |
| --- | ---: | ---: | ---: |
| CSS | 85.44 kB | 85.44 kB | 0.00 kB / 0.00% |
| CSS gzip | 20.36 kB | 20.36 kB | 0.00 kB / 0.00% |
| Main JavaScript | 656.18 kB | 657.34 kB | +1.16 kB / +0.18% |
| Main JavaScript gzip | 189.63 kB | 189.94 kB | +0.31 kB / +0.16% |
| Main JavaScript source map | 2,405.76 kB | 2,410.08 kB | +4.32 kB / +0.18% |
| Linux release executable | 18,600,144 bytes | 18,601,552 bytes | +1,408 bytes / +0.008% |
| Debian package | 6,563,164 bytes | 6,564,742 bytes | +1,578 bytes / +0.024% |
