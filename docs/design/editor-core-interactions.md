...
Find in Files carries all four options through its cancellable request identity and native bounded-search contract. In the command surface, New line is an editing action that inserts one line break into the query field at the caret; the field grows with its own text, and a query that contains a real line break is searched across lines and makes replacement read-only. Files and Recent present the same four in-field controls, where Match case, Whole words, and Regex select the quick-open path matcher. The Quick Open header also exposes a default-on Git-ignore filter across Files, Recent, Text, and Commands. Turning it off permits bounded navigation and text search of catalogued ignored files. Those files can be opened, edited, and saved through their session identities, while bulk replacement remains rejected at both the presentation and workspace boundaries. This extends [`ADR-0005`](../architecture/decisions/0005-bounded-navigation-search.md) and [`ADR-0017`](../architecture/decisions/0017-editable-ignored-files.md) without adding an index, startup scan, or new mutation authority.
...
Project selection resolves the canonical folder before deciding where to open it. When that root is already active or belongs to a native window that is still starting, Asterlyn restores and focuses the existing window instead of showing the placement dialog or creating another copy. Pending paths remain reserved after their one-time startup read until activation completes, so two requests cannot create parallel windows during the startup gap. Failed construction, failed activation, project replacement, and window destruction release their exact window identity. The visible information message distinguishes this focus route from successful creation of a new window.
...
An editable working Diff may retain an ephemeral text buffer for editing, file watching, and save coordination. That backing buffer stays internal to the Diff and is omitted from both the tab strip and the open-tab menu. Explicitly opening the source promotes the same buffer to an ordinary visible text tab; closing the Diff discards a clean backing buffer and promotes a dirty one so unsaved content always retains a visible editor entry.
...
relative path as the secondary title. Navigating to a change marks the complete contiguous change
block on every visible side with one high-contrast perimeter and accent edge, in addition to
centering its first row. Insertions and deletions include the aligned empty rows on the opposite
