...
- **Diff presentation:** Git returns one bounded canonical patch. Unified view displays it directly; split view derives aligned before/after documents in the presentation layer and synchronizes vertical position proportionally plus horizontal position by source offset. Whitespace visibility is a CodeMirror decoration over the same patch, never a different Git query, so display preferences cannot alter review truth.
...
The frontend requests a repository snapshot with a monotonically increasing request generation. The backend returns immutable data. Repository refresh is deliberately two-phase: tracked/index, current-`HEAD` history, and ref state arrive first; an independently cancellable untracked-path scan completes the snapshot afterward. The UI identifies pending untracked discovery instead of briefly claiming the repository is clean, merges a supplement only into its matching generation, and cancels obsolete Git child processes when the repository changes or a mutation begins.
...
Ref history outside the current `HEAD` is also a separate lazy read path. The Rust boundary accepts only complete literal refs from the displayed local, remote-tracking, and tag namespaces, verifies the ref, resolves it to a commit object, and queries the resulting object ID with an explicit path separator. The frontend rejects responses whose request generation, repository root, or selected ref no longer matches. Collapsing a ref group is presentation state only and cannot alter the selected ref, history data, repository truth, or active editor document.
