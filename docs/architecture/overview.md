...
- **Editor engine:** CodeMirror is wrapped as an editor surface. Document models, commands, diagnostics, and navigation must not leak CodeMirror classes across the application boundary. Basic syntax highlighting matches a bounded file's name inside that adapter, loads the catalog and selected parser on demand, rejects stale asynchronous completions, and retains plain-text editing on failure; it is not language intelligence.
- **Workspace files:** the Git capability authorizes a root-qualified project-file identity against a fresh bounded catalog, while the workspace capability alone owns secure traversal, text decoding, content revisions, atomic replacement, and the private recovery-journal format. Tauri supplies the application-local recovery directory, serializes workspace writes, and translates identities, but cannot become the filesystem policy owner. Recovery blobs are bounded exact-byte safety data outside the repository, never repository truth.
