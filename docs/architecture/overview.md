...
- **Workspace files:** the Git capability authorizes a root-qualified project-file identity against a fresh bounded catalog, while the workspace capability alone owns secure traversal, text decoding, content revisions, and replacement. Tauri translates and coordinates those capabilities but cannot become the filesystem policy owner.
