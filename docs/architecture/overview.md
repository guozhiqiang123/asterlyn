...
- **Navigation and search:** file/command ranking and repository-scoped recents remain pure presentation state over the bounded project catalog. Workspace text search is a lazy, cancellable application service that regenerates the Git-authorized candidate set before delegating bounded literal matching to the workspace capability. It creates no startup index or parallel repository database, and active-buffer replacement continues through CodeMirror plus the E1 save boundary.
