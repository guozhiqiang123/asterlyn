...
- **Workspace files:** the Git capability authorizes a root-qualified project-file identity against a fresh bounded tracked-and-non-ignored-untracked catalog, while the workspace capability alone owns secure traversal, text decoding, content revisions, atomic replacement, and the private recovery-journal format. The Files tool may additionally request a bounded display-only list of Git-ignored files and collapsed directories for visual context. Those entries never enter read, save, search, or replacement authorization. Tauri supplies the application-local recovery directory, serializes workspace writes, and translates identities, but cannot become the filesystem policy owner. Recovery blobs are bounded exact-byte safety data outside the repository, never repository truth.
