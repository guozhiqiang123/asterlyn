...
The three Git columns are also independent render regions. Selecting a commit patches only the old and new history-row selection state, then replaces the detail column when its root-qualified result arrives; selecting a commit file patches only file-row selection and the editor document. Neither action may recreate the branch tree, history toolbar, history list, or their scroll containers. Commit file lists are immutable for a root-qualified object ID and may use a session-only, 48-entry least-recently-used cache. Async generation and repository-selection checks remain authoritative on both cache misses and completions.

The window header presents only the active project's display name plus a disclosure control. Its absolute path is tooltip metadata rather than permanent chrome. The project menu contains the system-folder Open action and at most eight successfully opened recent projects, each with its display name, absolute path, and an explicit remove-history action; the active project is already represented by the header and is not repeated in an `Open Projects` section. Opening the menu performs one bounded native existence check for those visible history entries. A path confirmed missing or no longer a directory is removed from persisted history, while permission and other inconclusive filesystem failures retain the entry to avoid losing temporarily unavailable local or mounted projects. Re-rendering while the menu remains open does not repeat the same check. Left-tool headers contain one primary title with the bounded item count immediately adjacent. Every independently hideable tool window exposes a named Hide button at its top-right edge and routes that action through the same persisted layout transition as its activity-rail control. Hiding a tool cannot close, replace, or reload the active editor document.

A newly activated project root starts with every directory collapsed, regardless of catalog depth.
This bounds the first mounted tree and gives large repositories a predictable overview. Disclosure
is still path-keyed presentation state: reconciliation of the same root preserves surviving expanded
directories, explicit locate/expand actions may open them, and switching to a different root clears
the previous project's disclosure identities.
...
U6 keeps the activity rail fixed-width. Every content boundary is resizable by pointer and keyboard: left dock/editor, editor/bottom dock, the two internal Branches dividers, and the side-by-side Diff divider. Pointer-move bursts retain only the latest value for each animation frame. A pointer gesture captures its validated range once before the first write, avoiding a layout-forcing width/height read after every frame; keyboard and new pointer gestures recompute current bounds. The applied value updates the affected CSS layout property directly; only dimensions that change the editor viewport request one coalesced editor measurement. The Git panes and fixed-height history rows isolate layout and paint work, and supported webviews skip offscreen row rendering while retaining the complete bounded accessibility and scroll model. The splitter controller reports an explicit drag lifetime, observes movement and release at the window boundary, and finalizes lost pointer capture. The Files pane suppresses its composited overlay scrollbar only during a left-divider drag, then restores it at the final pane width; this prevents a stale macOS WebKit scrollbar layer from visually separating from the divider. Resizing changes layout state only and never refreshes repository data.
...
Each of the at most 20 open text tabs retains one CodeMirror `EditorState`, including its document tree, selection, history, language compartment, and last scroll offsets. Only the active tab owns an `EditorView` and mounted editor DOM. Switching between ordinary text tabs reuses that same view by replacing its state; changing to a Diff, Markdown presentation, loading, or error surface releases the view. Closing a tab, replacing its load epoch, or leaving the repository releases its retained state. Opening a twenty-first file retires the oldest inactive clean tab instead of refusing navigation; dirty buffers and in-flight saves are never automatic eviction candidates, and navigation is rejected only when every retained buffer requires protection. Parser installation occurs after the plain document has had a paint opportunity and remains activation-guarded, so leaving an XML or other parser-backed file cancels its pending installation instead of letting obsolete UI-thread parsing delay the next selected file. A tab activation updates the editor and tab strip without rebuilding the project-tree DOM. Exact-line-ending edits update the affected normalized text and separator slice directly, while application-state notification is coalesced to one animation frame and flushed synchronously before save, close, or activation.
...
Repository-independent editor typography persists only bounded presentation and input values. Font family, size, line-height factor, letter spacing, visual tab width, and inserted-space indentation apply without rewriting the current document. Asterlyn ships the upstream JetBrains Mono variable font from its pinned public Fontsource distribution under the SIL Open Font License 1.1, rather than relying on a same-named operating-system installation or extracting the IDE/JBR copy. The cross-platform baseline remains 14 pixels, 1.35 line height, normal zero letter spacing, 400 weight with discretionary/code ligatures disabled, four-space indentation, and four-column tabs. These numeric values are visually calibrated for the CodeMirror/WebView renderer rather than claimed to be numerically identical to Android Studio's JBR font metrics. CJK and other uncovered glyphs retain an explicit system-monospace fallback. Letter spacing is an Asterlyn extension because the JetBrains editor exposes line height but not editor character spacing. Language- and `.editorconfig`-specific policy remains an E3.3 adapter concern.

Cascadia Code, Fira Code, Source Code Pro, and IBM Plex Mono are explicit optional choices, not bundled font payloads. Selecting one fetches one pinned Latin WOFF2 asset from the fixed jsDelivr npm origin, rejects redirects and files over 256 KiB, verifies an embedded SHA-256 digest before constructing a `FontFace`, and attempts to cache the verified bytes in profile storage. A failed download or font decode retains the previously active family and offers retry; a cached file is verified again before reuse. If profile caching alone fails, the verified face remains available in the current window and the UI reports that it was not cached. The content-security policy permits only the fixed CDN origin for this presentation-owned fetch, and preferences persist a closed font identifier rather than a path or URL. Version-five preferences add that identifier while older profiles migrate to bundled JetBrains Mono. Each native window owns its loaded browser font face while sharing the bounded preference and verified profile cache when storage is available.
...
16. Commit or commit-file selection cannot replace the history scroll container; query, paging, topology, and repository changes may rebuild history rows when their projection changes.
