...
U1–U3 are locally accepted as one usability phase. Asterlyn now has compact desktop chrome; searchable, keyboard-navigable commit file/patch inspection; filtered scope-aware working-tree multi-selection and batch stage/unstage; and shared unified/split/whitespace patch presentation. The local evidence and known limits are recorded in the interaction backlog. This phase is ready for one consolidated remote push and cross-platform preview build. U4 safe local branch work is the next active product slice after that checkpoint.
