...
U8 is the active Diff and ref-navigation consistency slice. It links horizontal as well as vertical split-Diff movement, makes a selected local/remote/tag ref the source of the middle history column, and adds semantic collapse controls for ref groups. Exact-ref validation, stale-result rejection, focused Git tests, and visible interaction acceptance are required before this slice is accepted.
