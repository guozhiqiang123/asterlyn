...
U1–U3 are published as the first usability phase. Asterlyn now has compact desktop chrome; searchable, keyboard-navigable commit file/patch inspection; filtered scope-aware working-tree multi-selection and batch stage/unstage; and shared unified/split/whitespace patch presentation. U4 safe local branch work is also locally accepted: existing local checkout and new local-branch creation are available only behind a complete clean-worktree gate, with the Git core repeating validation immediately before mutation. The evidence and explicit limits are recorded in the interaction backlog. U5 remote daily-loop work is the next active product slice after the U4 phase checkpoint.
