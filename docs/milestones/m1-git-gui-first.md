...
U11.1 is the active interaction correction before Stage 3. It separates Branch quick navigation from multi-ref editing, replaces the one-path scrolling menu with multiline and tree-based exact path selection, and introduces root-qualified read identities for the main repository and initialized submodules. The stage cannot close until ref/path/root combinations, colliding object IDs, commit details, Diff routing, stale-root rejection, compact interaction, accessibility, and the unchanged top-level-only mutation boundary are accepted together. Packaging and remote publication remain one checkpoint after the whole correction.
