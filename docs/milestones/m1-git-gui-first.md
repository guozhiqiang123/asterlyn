...
Feature development now follows the [`daily-driver interaction backlog`](../design/daily-driver-interactions.md). U1–U3 take priority over platform-specific release polish: first remove duplicated window chrome, then complete per-commit file/patch inspection, then improve large working-tree review. Open Windows/macOS interactive checks still prevent a release-candidate claim, but they do not block these usability slices.

U1 is locally accepted: Asterlyn now uses one compact draggable application bar with accessible window controls instead of stacking it below the native title row. U2 is also locally accepted: searchable history now opens a selected commit into changed files and a first-parent/root-aware read-only patch, with stale-result rejection, designed asynchronous states, and keyboard traversal. The local evidence and known limits are recorded in the interaction backlog. U3 scalable working-tree review is the next active product slice; U1–U3 will be pushed and packaged together after that larger usability phase rather than triggering remote builds for every small step.
