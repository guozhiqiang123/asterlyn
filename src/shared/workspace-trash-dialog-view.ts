...
export function renderWorkspaceTrashDialog<TTarget extends WorkspaceTrashTarget>(
  ...
  return `<section class="dialog project-files-dialog" role="alertdialog" aria-modal="true" aria-labelledby="workspace-trash-dialog-title" aria-describedby="workspace-trash-dialog-detail">
    <div class="dialog-heading"><div><span class="panel-eyebrow">${escapeHtml(copy.eyebrow)}</span><h2 id="workspace-trash-dialog-title">${escapeHtml(copy.title)}</h2></div><button class="icon-button" data-workspace-trash-close type="button" aria-label="${escapeAttribute(copy.cancel)}" ${dialog.busy ? "disabled" : ""}>×</button></div>
