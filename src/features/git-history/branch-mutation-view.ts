...
export function renderBranchMutationDialog(
  ...
  if (!dialog.plan) {
    const name = mutationNeedsName(kind)
      ...
      : "";
    return `<section class="dialog branch-mutation-dialog" role="dialog" aria-modal="true" aria-labelledby="branch-mutation-dialog-title" aria-describedby="branch-mutation-description">${heading}<p id="branch-mutation-description">${escapeHtml(copy.descriptions[kind])}</p><div class="branch-mutation-source"><span><small>${escapeHtml(copy.source)}</small><strong>${escapeHtml(dialog.source.fullName)}</strong></span><code>${escapeHtml(dialog.source.oid.slice(0, 12))}</code></div>${error}<form id="branch-mutation-review-form" class="branch-mutation-form">${name}<div class="dialog-actions"><button class="secondary-button" data-branch-mutation-close type="button" ${dialog.busy ? "disabled" : ""}>${escapeHtml(copy.cancel)}</button><button class="primary-button" type="submit" ${dialog.busy ? "disabled" : ""}>${escapeHtml(dialog.busy ? copy.reviewing : copy.review)}</button></div></form></section>`;
  }
