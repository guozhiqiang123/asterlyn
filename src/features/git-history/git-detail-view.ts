...
export function renderBranchDetail(model: BranchDetailViewModel): string {
  ...
  const blockers = safety.blockers.length
    ...
    : "";
  return `<div class="inspector-header"><span class="panel-eyebrow">${escapeHtml(branch.kind)}</span><h2>${escapeHtml(branch.name)}</h2></div><dl class="metadata-list"><div><dt>State</dt><dd>${branch.current ? "Checked out" : "Available"}</dd></div><div><dt>Upstream</dt><dd>${escapeHtml(branch.upstream ?? "None")}</dd></div><div><dt>Tracking</dt><dd>${escapeHtml(branch.tracking ?? "No divergence")}</dd></div><div><dt>Updated</dt><dd>${formatRelative(branch.committedAt)}</dd></div></dl>${mainRoot && !branch.current ? `<div class="git-detail-operation-actions" role="group" aria-label="Operations using ${escapeAttribute(branch.name)}"><button class="secondary-button" type="button" data-start-git-operation="merge" data-operation-target="${escapeAttribute(branch.fullName)}">Merge into current</button><button class="secondary-button" type="button" data-start-git-operation="rebase" data-operation-target="${escapeAttribute(branch.fullName)}">Rebase current onto</button></div>` : ""}<section class="branch-action-card ${safety.ready ? "ready" : "blocked"}"><div class="branch-action-heading"><span>${icon("branch", 15)}</span><strong>Checkout</strong></div><p>${localTarget ? escapeHtml(safety.message) : mainRoot ? "Select a local branch to check it out. Remote and tag checkout remain deferred." : "This initialized submodule is available for history inspection only; branch mutations remain scoped to the main repository."}</p>${localTarget ? blockers : ""}<button class="primary-button" id="checkout-branch" type="button" ${canCheckout ? "" : "disabled"}>${escapeHtml(checkoutLabel)}</button></section>${mainRoot ? `<section class="branch-action-card create-branch-card ${safety.ready ? "ready" : "blocked"}"><div class="branch-action-heading"><span>${icon("plus", 15)}</span><strong>New local branch</strong></div><p>Create from the current <code>HEAD</code>. The same clean-worktree gate applies.</p><form id="create-branch-form"><label for="new-branch-name">Branch name</label><input id="new-branch-name" type="text" value="${escapeAttribute(model.newBranchName)}" placeholder="feature/name" autocomplete="off" spellcheck="false" /><button class="secondary-button" id="create-branch-button" type="submit" ${safety.ready && model.newBranchName.trim() && !model.loading ? "" : "disabled"}>Create and checkout</button></form></section>` : ""}`;
}
