...
function renderPushDialog(model: RemotePushDialogViewModel): string {
  ...
  const modeMenuAvailable = Boolean(
    preview &&
    actionable &&
    (preview.ordinaryAllowed || preview.forceWithLeaseAllowed) &&
    !operation &&
...
function renderPushDiffResizeHandles(label: string): string {
  return (["n", "ne", "e", "se", "s", "sw", "w", "nw"] as const)
    .map((edge) => `<span class="push-diff-resize-handle ${edge}" data-push-diff-resize="${edge}" ${edge === "se"
      ? `role="separator" tabindex="0" aria-label="${escapeAttribute(label)}"`
      : 'aria-hidden="true"'}></span>`)
    .join("");
}
