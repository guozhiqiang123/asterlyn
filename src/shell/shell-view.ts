    ...
    <header class="topbar" data-tauri-drag-region>
      ...
      <div class="topbar-actions" data-tauri-drag-region>
        ...
        <div class="remote-toolbar git-unavailable" id="remote-toolbar" role="group" aria-label="${escapeHtml(copy.remoteActions)}">
          ...
          <div class="remote-toolbar-menu-anchor" id="remote-toolbar-menu-anchor"><button class="icon-button" id="remote-toolbar-menu-toggle" type="button" aria-label="${escapeHtml(copy.moreRemoteActions)}" title="${escapeHtml(copy.moreRemoteActions)}" aria-haspopup="menu" aria-controls="remote-toolbar-menu" aria-expanded="false">${icon("more", 18)}</button><div class="remote-toolbar-menu hidden" id="remote-toolbar-menu" role="menu" aria-label="${escapeHtml(copy.moreRemoteActions)}"><button class="remote-toolbar-menu-item remote-action-button" id="remote-fetch" type="button" role="menuitem" data-remote-action="fetch" aria-label="${escapeHtml(copy.fetchBranch)}" title="${escapeHtml(copy.fetchBranch)}" aria-disabled="true">${icon("download", 17)}<span>${escapeHtml(copy.fetchBranch)}</span></button></div></div>
          <span class="topbar-remote-action" id="remote-update-hint"><button class="icon-button remote-action-button" id="remote-update" type="button" data-remote-action="pull" aria-label="${escapeHtml(copy.updateBranch)}" title="${escapeHtml(copy.updateBranch)}" aria-disabled="true">${icon("download", 18)}</button></span>
          <span class="topbar-remote-action" id="remote-push-hint"><button class="icon-button remote-action-button" id="remote-push" type="button" data-remote-action="push" aria-label="${escapeHtml(copy.pushBranch)}" title="${escapeHtml(copy.pushBranch)}" aria-disabled="true">${icon("upload", 18)}</button></span>
