    ...
    <div class="workspace" id="workspace">
      ...
      <section class="workbench" id="workbench">
        ...
        <section class="bottom-tool tool-window" id="bottom-tool" aria-label="Branches and Git log">
          <div class="bottom-tool-header"><strong>Git</strong><span>Branches and Log</span><button class="compact-icon-button git-operation-open" id="git-operation-open" type="button" aria-label="Prepare a Merge, Cherry-pick, Rebase, or Squash operation" title="Git operations…">${icon("more", 15)}</button><button class="compact-icon-button" id="git-recoveries-open" type="button" aria-label="Recover local changes" title="Recover local changes">${icon("revert", 15)}</button><button class="bottom-tool-hide" id="hide-git-tool" type="button" aria-label="Hide Git tool window" title="Hide Git tool window">${icon("close", 14)}</button></div>
