...
pub(crate) use git_recovery::{
    list_git_worktree_recoveries, prepare_restore_changes, undo_git_worktree_recovery,
};
mod git_reads;
mod image;
mod shell;
...
pub(crate) use git_operations::{
    ...
    execute_git_operation, fetch_remote, prepare_git_operation, pull_current, push_current,
    ...
    stage_paths, store_remote_https_credential, switch_branch, unstage_paths,
};
...
pub(crate) use shell::{
    initial_repository, open_project, open_repository_window, read_project_snapshot,
    window_chrome_mode,
};
